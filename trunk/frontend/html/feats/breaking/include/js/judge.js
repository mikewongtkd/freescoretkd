app.network.on
	...
	.by( update => {
		...
		if( app.state.current.athleteid == id ) { app.refresh.tool.deductions( division ); }
		app.sound.ok.play();
	})
	.handle( 'navigate' )
	.by( update => {
	...
	.by( update => {
		...
		app.refresh.tool.deductions( division );
		app.refresh.tool.scoring( division );
		app.refresh.tool.inspection( division );
		app.refresh.tool.help( division );
	})
	.handle( 'score' )
	.by( update => {
		let request   = update.request
