...
refresh.checkin = ( registration ) => {
	...
	var none_checked_in = ( a, b ) => {
		...
		var athleteB = b in registration.athletes ? registration.athletes[ b ] : { checkedin : true };
	...
	var num_checked_in = ( a, b ) => {
		var athleteA = Number.isInteger( a ) ? a : (registration.athletes[ a ].checkedin ? 1 : 0);
		var athleteB = Number.isInteger( b ) ? b : (registration.athletes[ b ].checkedin ? 1 : 0);
	...
	$( '#athletes-confirmed .btn-group,.list-group' ).empty();

	$( '#athletes-registered .list-group' ).btsListFilter( '#search-registered', { initial: false, resetOnBlur: false });
	$( '#athletes-confirmed .list-group' ) .btsListFilter( '#search-confirmed',  { initial: false, resetOnBlur: false });
	...
	Object.keys( registration.athletes )
		...
		.forEach(( key ) => {
			...
			var list      = list = checkedin ? $( '#athletes-confirmed .list-group' ) : $( '#athletes-registered .list-group' );
			if( list.children( '#' + letterid ).length == 0 ) {
				...
				list = checkedin ? $( '#athletes-confirmed .btn-group' ) : $( '#athletes-registered .btn-group' );
				list.append( scrollto );
			}

			list = checkedin ? $( '#athletes-confirmed .list-group' ) : $( '#athletes-registered .list-group' );
	...
	registration.event.order.forEach(( ev ) => {
		...
		Object.keys( registration.events[ ev ]).forEach(( divid ) => {
			...
			var n            = html.div.clone().addClass( 'pull-right' ).append( div.athletes.reduce( num_checked_in ), '/', div.athletes.length );
