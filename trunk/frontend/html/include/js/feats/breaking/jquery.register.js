$.widget( "freescore.register", {
	...
	_create: function() {
		...
		// ------------------------------------------------------------
			if( defined( o.url ) && ( o.url.match( /judge|coordinator/ ) != null)) {
				...
				} else if( o.url.match( /coordinator/ ) != null ) {
		...
		// ------------------------------------------------------------
		register.roles.add = function( name, imageSrc, left ) {
		// ------------------------------------------------------------
			var dom   = html.div.clone() .addClass( "role" ) .attr( "role", name ) .css( "top", 0 ) .css( "left", left );
			var img   = html.img.clone() .attr( "src", imageSrc ) .attr( "height", 100 );
		...
		// ------------------------------------------------------------
			...
			var img   = html.img.clone() .attr( "src", "../../images/roles/judge.png" ) .attr( "height", 100 );
		...
		// ------------------------------------------------------------
			...
			if( role == "judge" ) {
				...
				url  = `./coordinator.php?ring=${$.cookie( "ring" )}`;
				role = register.roles.add( "Coordinator", "../../images/roles/coordinator-breaking.png", '200px' );
			}
			url = url.replace( /\/\/+/g, "/" );
		...
		register.roles.data.push( register.roles.add( 'Judge',       '../../images/roles/judge.png',                '0px'   ));
		register.roles.data.push( register.roles.add( 'Coordinator', '../../images/roles/coordinator-breaking.png', '200px' ));
