$.widget( "freescore.judgeController", {
	...
	_create: function() {
		...
		let btn = $.fn.button.noConflict() // reverts $.fn.button to jqueryui btn
		$.fn.btn = btn // assigns bootstrap button functionality to $.fn.btn
		$.fn.presentationWidget = $.fn.presentationButtons // Default to presentation buttons

		// ===== INITIALIZE WIDGET ELEMENTS
		let card         = e.card         = html.div.clone() .addClass( "card" );
		let front        = e.front        = html.div.clone() .addClass( "front" );
		let views        = e.views        = html.div.clone() .addClass( "view control-group" );
		let controllers  = e.controllers  = html.div.clone() .addClass( "controller control-group" );

		let flipToBack   = e.flipToBack   = html.div.clone() .addClass( "flip" ) .html( "Division List" );
		let info         = e.info         = html.div.clone() .addClass( "division-info" ) .html( '<div class="label">Current Division</div>' );
		let description  = e.description  = html.div.clone() .addClass( "description" );
		let round        = e.round        = html.div.clone() .addClass( "round" );
		let form         = e.form         = html.div.clone() .addClass( "form" );
		let athlete      = e.athlete      = html.div.clone() .addClass( "athlete" );
		let score        = e.score        = html.div.clone() .addClass( "score" );
		let accuracy     = e.accuracy     = html.div.clone() .addClass( "accuracy" );
		let presentation = e.presentation = html.div.clone() .addClass( "presentation" );
		let total        = e.total        = html.div.clone() .addClass( "total" );
		let matPosition  = e.matPosition  = html.div.clone() .matposition();

		let major        = e.major        = html.div.clone() .deductions({ value : 0.3, controller : this });
		let minor        = e.minor        = html.div.clone() .deductions({ value : 0.1, controller : this });
		let controls     = e.controls     = html.div.clone() .addClass( "controls" );

		let label        = e.label        = html.div.clone() .addClass( "control-label" ) .html( "Presentation Score" );
		let power        = e.power        = html.div.clone() .presentationWidget({ name : 'power',  label : 'Power and Speed',      controller: this });
		let rhythm       = e.rhythm       = html.div.clone() .presentationWidget({ name : 'rhythm', label : 'Rhythm and Control',   controller: this });
		let ki           = e.ki           = html.div.clone() .presentationWidget({ name : 'energy', label : 'Expression of Energy', controller: this });
		let send         = e.send         = html.div.clone() .button({ label : "Send" }) .addClass( "send" );

		// ===== UPDATE BEHAVIOR
		widget.on( "updateRequest", function( ev ) {
			let o = ev.score;
			let e = ev.score.elements;
		...
		let back         = e.back         = html.div.clone() .addClass( "back" );
		let notes        = e.notes        = html.div.clone() .judgeNotes({ num : o.num });
		let flipToFront  = e.fliptoFront  = html.div.clone() .addClass( "flip" ) .html( "Return to Scoring" );
		let nav          = e.nav = {
			athlete : {
		...
		let flipDisplay  = e.flipDisplay  = html.div.clone() .button({ label : "Leaderboard" }) .addClass( "navigate nav-button display-mode" ) .attr( 'action', 'display' ) .attr( 'sound', 'ok' );
	...
	_init: function( ) {
		...
		let o           = this.options;
		let html        = e.html;
		let ws          = e.ws = new WebSocket( o.server ); 
		let network     = { reconnect: 0 };
		...
		ws.onmessage = network.message = function( response ) {
			let update   = JSON.parse( response.data ); if( ! defined( update.division ) || update.type != 'division' || update.action != 'update' ) { return; }
			console.log( update );
			...
			let button = {
				enable : function( button ) {
					...
					button.off( 'click' ).click( function() {
						let request  = { data : { type : button.attr( 'type' ), action : button.attr( 'action' ) }}; 
						request.json = JSON.stringify( request.data );
						ws.send( request.json ); 
			...
			e.flipDisplay .off( 'click' ) .click( function() {
				let request  = { data : { type : 'division', action : 'display' }}; 
				request.json = JSON.stringify( request.data );
				ws.send( request.json ); 
				e.sound.ok.play();
			});
			...
			if( division.form.count() <= 1 ) { // One form; no need to navigate
				...
				let first = division.current.form.is.firstForm();
				let last  = division.current.form.is.lastForm();
			...
			// ----------------------------------------
			let numRounds = division.round.count();
			if       ( numRounds == 1 ) { 
