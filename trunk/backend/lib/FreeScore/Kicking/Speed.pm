package FreeScore::Kicking::Speed;
use List::MoreUtils qw( first_index );
use FreeScore;
use FreeScore::Forms;
use FreeScore::Kicking::Speed::Division;
use base qw( FreeScore::Forms Clone );

our $SUBDIR = "speed-kicking";
...
# ============================================================
	...
	if( defined $ring ) { 
		...
		foreach my $id (@$divisions) {
			push @{ $self->{ divisions }}, new FreeScore::Kicking::Speed::Division( $self->{ path }, $id, $ring );
		}
		...
		$self->{ divisions } = [];
		foreach my $id (@$divisions) {
			push @{ $self->{ divisions }}, new FreeScore::Kicking::Speed::Division( $self->{ path }, $id );
		}
...
# ============================================================
	my $self     = shift;
	my $divid    = shift; 
	my $division = new FreeScore::Kicking::Speed::Division( $self->{ path }, $divid );
